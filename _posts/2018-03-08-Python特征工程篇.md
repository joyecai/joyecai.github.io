...
```python
##本段代码来自 https://zhuanlan.zhihu.com/p/38034501
##利用特征重要性筛选特征
import pandas as pd
...
for i in range(100,all_num,100):
    ...
    if break_num==2:
        break
print('筛选出来最佳特征个数为',best_num)
```

> 内存原因，没办法一次性加载所有特征，故对每组特征进行组内筛选，对组内特征进行重要性排序，
> 排序完筛选办法有两种，一种是按照排名进行前向搜索，另一种就是直接测试前n*5(n位正整数)个特征的效果，
> 由于多数统计特征之间相关性很高，大概每组30+个特征的信息就能代表整组特征所包含的信息。第一种精度会好一些但速度慢，第二种办法就相对快很多。
