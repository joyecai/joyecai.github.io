...
### 特征工程（进阶）
	...
	for feat in vector_feature:                    #长度
		data['len_'+feat] = data[feat].apply(lambda x:0 if ((not x.strip()) or str(x)=='-1') else len(x.split(' ')))
	
	data['meanlen_interest'] = (data['len_interest1']+data['len_interest2']+data['len_interest5'])/3.0    ##平均
	data['max_ct'] = data['ct'].apply(lambda x: 0 if ((not str(x).strip()) or str(x)=='-1') else max(int(i) for i in str(x).split(' ')))  ##最大值
