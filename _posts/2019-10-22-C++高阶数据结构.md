...
强化学习本质就是动态规划，Graph Embedding本质就是图论（dfs的随机游走，bfs的GCN），掌握本质才能迸发出大的创新。
...
Splay树核心过程：Splay(x, y)，即将x节点转移到y节点的子节点上面（其中y是x的祖先）。

利用其中双旋的优势能够保证查询复杂度均摊为O(log n)。
...
> [左偏树的详细介绍与实现](https://blog.csdn.net/theprinceofelf/article/details/7247454)
...
[【POJ 3016】K-Monotonic](http://poj.org/problem?id=3016)
