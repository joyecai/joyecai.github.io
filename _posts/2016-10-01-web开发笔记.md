...
编码显示问题：
- vi ~/.vimrc     #编辑：set fileencoding=utf-8
- source ~/.vimrc #激活

#### 字符编码

- ASCII：python默认编码，只能表示数字、英文字母和一些特殊符号，不能表示汉字
- unicode：用于内存中存储，可表示汉字，固定长度
- utf-8：用于磁盘文件存储，可表示汉字，可变长度
...
- 访问并摘取一部分：/hadoop fs `-text` .../* | head -n 100 >> sample.txt
